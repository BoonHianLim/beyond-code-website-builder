...
export const ExpressErrorHandler = (err: unknown, req: Request, res: Response, next: NextFunction) => {
	...
	logger.error('default ExpressErrorHandler error', err, err instanceof Error ? err.stack : undefined)
...
export const ErrorHandling = (err: unknown, res: Response) => {
	switch (true) {
		case err instanceof StatusCodeError:
			res.status((err as StatusCodeError).status).json({ message: (err as StatusCodeError).message })
			break
